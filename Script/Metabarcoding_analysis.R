...
ps_genus <- tax_glom(ps, taxrank = "Genus") # Agrupar a nivel de genero
# Sustituir g__Allorhizobium-Neorhizobium-Pararhizobium-Rhizobium por g__Rhizobium
tax_table(ps_genus)[tax_table(ps_genus)[, "Genus"] == "g__Allorhizobium-Neorhizobium-Pararhizobium-Rhizobium", "Genus"] <- "g__Rhizobium"
tax_table(ps_genus)[tax_table(ps_genus)[, "Genus"] == "g__Methylobacterium-Methylorubrum", "Genus"] <- "g__Methylorubrum"
...
#----SEED Alfa_Diversity_DomesticationStatus----
...
p
#----SEED Alfa_Diversity_PloidyLevel----
...
#----GNOTO TaxabarPlot----
ps_gno_g<- subset_samples(ps_genus, gnotobiotic == "Yes") # Filtrar para quedarte solo con las muestras tipo "Seed"
ps_gno_rel <- transform_sample_counts(ps_gno_g, function(x) x / sum(x))
...
#----FIELD TaxabarPlot----
ps_field_g<- subset_samples(ps_genus, Field == "Yes") # Filtrar para quedarte solo con las muestras tipo "Seed"
...
# Flag rows as top genus or "Other"
df <- df %>%
  mutate(Genus_filtered = ifelse(Genus %in% top_genera, Genus, "Other"))

df_grouped <- df %>%                                         # sum per sample first
  group_by(Sample, WheatSpecies, PlantPart, Genus_filtered) %>% 
  summarise(total_abund = sum(Abundance), .groups = "drop")

df_summary <- df_grouped %>%                                 # mean across samples
  group_by(WheatSpecies, PlantPart, Genus_filtered) %>% 
  summarise(mean_abund = mean(total_abund), .groups = "drop")

genus_order <- df_summary %>%                     # order by global mean abundance
  group_by(Genus_filtered) %>% 
  summarise(global_mean = mean(mean_abund), .groups = "drop") %>% 
  arrange(global_mean) %>% 
  pull(Genus_filtered)

if (!"Other" %in% genus_order) genus_order <- c("Other", genus_order)

df_summary <- df_summary %>%
  mutate(
    Genus_filtered = factor(Genus_filtered, levels = genus_order),
    PlantPart      = factor(PlantPart, levels = c("Soil", "Root", "Shoot", "Spike", "Seed"))
  )
Genus.palette <- c(
  Other            = "#E0E0E0",
  Bacillus         = "#E78AC3",
  Leifsonia        = "#A6D854",
  Hymenobacter     = "#B57EDC",
  Mycobacterium    = "#4a0c6b",
  Bacteroides      = "#3288bd",
  Pseudarthrobacter= "#FFD92F",
  Nocardioides     = "#8DA0CB",
  Staphylococcus   = "#d9ef8b",
  Massilia         = "#b695c0",
  Curtobacterium   = "#fc8d59",
  Rhizobium        = "darkgreen",
  Enterobacter     = "#91cf60",
  Sphingomonas     = "#fee08b",
  Pseudomonas      = "#ffffbf",
...
ordered_levels <- names(Genus.palette)   # vector con el orden exacto

df_summary <- df_summary %>%
  mutate(
    Genus_filtered = factor(Genus_filtered, levels = ordered_levels),
    PlantPart      = factor(PlantPart, levels = c("Soil", "Root", "Shoot", "Spike", "Seed"))
  )

ggplot(df_summary,
       aes(x = PlantPart, y = mean_abund, fill = Genus_filtered)) +
  geom_col(width = 0.8) +
  facet_grid(~WheatSpecies) +
  scale_fill_manual(values = Genus.palette,
...
df_pantoea_percent <- df_pantoea %>%
  group_by(WheatSpecies, PlantPart) %>%
  summarise(pantoea_abund = sum(Abundance), .groups = "drop") %>%
  group_by(WheatSpecies) %>%
  mutate(percent = round(100 * pantoea_abund / sum(pantoea_abund), 2)) %>%
  arrange(WheatSpecies, PlantPart)
...
#----FIELD LefSe----
lefse_results <- run_lefse(ps_field, group = "PlantPart")
df <- lefse_results@marker_table  # microbiomeMarker guarda los datos en esta ranura
df_tbl <- as.data.frame(marker_table(lefse_results))
df_tbl <- marker_table(lefse_results)
df_tbl <- as.data.frame(df_tbl)     # Esto convierte el contenido
class(df_tbl) <- "data.frame"
df_sep <- df_tbl %>%                       # tu data-frame original
  separate(
    col   = feature,
    into  = c("domain", "phylum", "class", "order", "family", "genus", "species"),
    sep   = "\\|",
    fill  = "right",      # si falta algún nivel, rellena con NA
    remove = FALSE        # TRUE si ya no quieres conservar la columna original
  )
df_sep <- df_sep %>%
  mutate(across(domain:species, ~ sub("^[a-z]__*", "", .)))
df_sep$pvalue_decimal <- format(df_sep$pvalue, scientific = FALSE)

group_colors <- c("Seed" = "saddlebrown", "Shoot" = "forestgreen", "Root"="skyblue", "Spike"= "yellow")
features_keep <- c(
  "Sphingomonas_s__","Pantoea_s__", "Massilia_s__", "Methylorubrum_s__", "Rhizobium_s__", "Pseudomonas_s__")
#  "d__Bacteria|p__Proteobacteria|c__Alphaproteobacteria|o__Sphingomonadales|f__Sphingomonadaceae|g__Sphingomonas|g__Sphingomonas_s__",
#  "d__Bacteria|p__Proteobacteria|c__Gammaproteobacteria|o__Enterobacterales|f__Erwiniaceae|g__Pantoea",
#  "d__Bacteria|p__Proteobacteria|c__Gammaproteobacteria|o__Enterobacterales|f__Erwiniaceae|g__Pantoea|g__Pantoea_s__",
#  "d__Bacteria|p__Proteobacteria|c__Gammaproteobacteria|o__Burkholderiales|f__Oxalobacteraceae|g__Massilia",
#  "d__Bacteria|p__Proteobacteria|c__Gammaproteobacteria|o__Burkholderiales|f__Oxalobacteraceae|g__Massilia|g__Massilia_s__",
#  "d__Bacteria|p__Proteobacteria|c__Alphaproteobacteria|o__Rhizobiales|f__Beijerinckiaceae|g__Methylorubrum"
#  "d__Bacteria|p__Proteobacteria|c__Alphaproteobacteria|o__Rhizobiales|f__Beijerinckiaceae|g__Methylorubrum|g__Methylorubrum_s__"
#)
df_sep <- df_sep[df_sep$species %in% features_keep, ]
#df_shoot <- df_sep[df_sep$enrich_group == "Shoot", ]
#df_shoot <- df_shoot %>%
#  filter(!is.na(species) & species != "")

p<-ggplot(df_sep, aes(x = genus, y = ef_lda, fill = enrich_group)) +
  geom_col(width = 0.7) +
  scale_fill_manual(values = group_colors) +
  coord_flip() +
  labs(
    title = "LEfSe Marker LDA Scores",
    x = "Feature",
    y = "LDA Score (log10)",
    fill = "Enriched in"
  ) +
  ...
  #+ ylim(0, max(df_filt$ef_lda) * 1.2)  # deja espacio para asteriscos

p
# Seleccionamos y preparamos los datos para heatmap
df_heat <- df_sep %>%
  select(species, enrich_group, ef_lda) %>%
  mutate(
    taxon = ifelse(!is.na(species) & species != "", species)
  ) %>%
  distinct(taxon, enrich_group, ef_lda)

p<- ggplot(df_heat, aes(x = taxon, y = enrich_group, fill = ef_lda)) +
  geom_tile(color = "white", width = 0.9, height = 0.9) +
  scale_fill_gradient2(low = "yellow", mid= "lightgreen", high = "darkgreen", midpoint = 4.5, name = "LDA Score") +
  labs(
    title = "LEfSe Biomarkers - Heatmap of LDA Scores",
    x = "Plant Part",
    y = "Taxa"
  ) +
  theme_minimal(base_size = 14) +
